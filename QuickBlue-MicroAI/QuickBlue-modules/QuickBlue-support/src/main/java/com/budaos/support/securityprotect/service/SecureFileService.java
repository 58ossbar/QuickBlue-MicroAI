...
public class SecureFileService {
    ...
    private static final List<String> ALLOWED_MIME_TYPES = Arrays.asList(
            "application/json", "application/zip", "application/x-7z-compressed", "application/pdf",
            "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            "application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation",
            "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            "application/vnd.ms-works", "text/csv", "audio/*", "video/*",
     ...
     * 检测文件安全类型
     */
    public ApiResult<String> checkFile(MultipartFile file) {
        ...
        if (level3ProtectConfigService.getMaxUploadFileSizeMb() > 0) {
            ...
            if (file.getSize() > maxSize) {
                return ApiResult.userErrorParam("上传文件最大为:" + level3ProtectConfigService.getMaxUploadFileSizeMb() + " mb");
            }
        }
        ...
        if (level3ProtectConfigService.isFileDetectFlag()) {
            ...
            if (ALLOWED_MIME_TYPES.stream().noneMatch(allowedType -> matchesMimeType(fileType, allowedType))) {
                return ApiResult.userErrorParam("禁止上传此文件类型");
            }
        }

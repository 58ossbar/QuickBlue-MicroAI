        ...
        <a-tab-pane key="1" tab="有效任务">
          <a-form class="qb-query-form">
            <a-row class="qb-query-form-row">
              ...
              <a-form-item label="触发类型" class="qb-query-form-item">
                <a-select style="width: 155px" v-model:value="queryForm.triggerType" placeholder="请选择触发类型" allowClear>
                  <a-select-option v-for="item in $enumPlugin.getValueDescList('TRIGGER_TYPE_ENUM')" :key="item.value" :value="item.value">
                    {{ item.desc }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              ...
              <a-form-item class="qb-query-form-item qb-margin-left10">
                ...
                <a-button class="qb-margin-left20" type="primary" @click="openUpdateModal()" v-privilege="'support:job:add'">
                  <template #icon>
                    <plus-outlined />
                  </template>
                  添加任务
          ...
          <a-flex align="end" vertical>
            <div class="qb-table-setting-block qb-margin-bottom10">
              <TableOperator
                class="qb-margin-bottom5 pull-right"
                v-model="columns"

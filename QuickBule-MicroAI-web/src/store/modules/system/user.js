...
export const useUserStore = defineStore({
  ...
  actions: {
    logout() {
      ...
      localRemove(localKey.USER_TENANT_ID);
      localRemove(localKey.USER_PLATFORM_ADMIN);
    ...
    setUserLoginInfo(data) {
      ...
      this.isPlatformAdmin = data.isPlatformAdmin === true;
...
function recursiveBuildMenuParentIdListMap(menuList, parentMenuList, menuParentIdListMap) {
  for (const e of menuList) {
    // 顶级parentMenuList清空（使用 == 兼容雪花ID字符串）
    if (e.parentId == 0) {
      parentMenuList = [];
    }
...
function buildMenuChildren(menu, allMenuList, visitedMenuIdSet = new Set()) {
  let menuIdStr = String(menu.menuId);
  // 防止菜单数据存在循环引用（parentId 指向自身或祖先）导致无限递归
  if (visitedMenuIdSet.has(menuIdStr)) {
    return;
  }
  visitedMenuIdSet.add(menuIdStr);
  ...
  for (const item of children) {
    buildMenuChildren(item, allMenuList, visitedMenuIdSet);
  }
}

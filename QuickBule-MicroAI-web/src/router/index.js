...
import { PAGE_PATH_404, PAGE_PATH_LOGIN, HOME_PAGE_PATH, PAGE_PATH_HELP_DOC, PAGE_PATH_INSTALL } from '/@/constants/common-const';
...
router.beforeEach(async (to, from, next) => {
  ...
  console.log('[Router] beforeEach:', {
    ...
    PAGE_PATH_INSTALL,
    startsWith: to.path.startsWith(PAGE_PATH_INSTALL)
  });
  ...
  // 帮助文档页面，免登录访问
  if (to.path.startsWith(PAGE_PATH_HELP_DOC)) {
    next();
    return;
  }

  // 安装向导页面，免登录访问
  if (to.path.startsWith(PAGE_PATH_INSTALL)) {
    console.log('[Router] 安装向导免登录通过');
    next();
    return;
  }
  ...
  // 首页（ 需要登录 ，但不需要验证权限）
  if (to.path === HOME_PAGE_PATH || to.name === HOME_PAGE_NAME) {
    next();
    return;
  }

...
<template>
  ...
  <a-row style="border-bottom: 1px solid #eeeeee; position: relative" v-show="pageTagFlag">
      ...
      <div class="qb-page-tag">
        <a-tabs style="width: 100%" :tab-position="mode" v-model:activeKey="selectedKey" size="small" @tabClick="selectTab">
          <a-tab-pane v-for="item in tagNav" :key="item.menuName">
            <template #tab>
              <span class="qb-page-tag-content">
                <home-outlined style="font-size: 12px" v-if="item.menuName === HOME_PAGE_NAME" class="qb-page-tag-close" />
                <component class="qb-page-tag-icon" v-else :is="$antIcons[item.menuIcon]" />
                {{ item.menuTitle }}
                <close-outlined @click.stop="closeTag(item, false)" v-if="item.menuName !== HOME_PAGE_NAME" class="qb-page-tag-close" />
              </span>
            </template>
          </a-tab-pane>
        </a-tabs>
    ...
    <a-dropdown>
      <!--标签页操作区-->
      <div class="qb-page-tag-operate">
        <div class="qb-page-tag-operate-icon">
          <AppstoreOutlined />
        </div>
      </div>
      <template #overlay>
...
@qb-page-tag-operate-width: 40px;
@color-primary: v-bind('token.colorPrimary');

.qb-page-tag-operate {
  width: @qb-page-tag-operate-width;
  height: @qb-page-tag-operate-width;
  background-color: #ffffff;
  font-size: 17px;
  text-align: center;
  vertical-align: middle;
  line-height: @qb-page-tag-operate-width;
  padding-right: 10px;
  cursor: pointer;
  color: #606266;

  .qb-page-tag-operate-icon {
    width: 20px;
    height: 20px;
    transition: all 1s;
  ...
  .qb-page-tag-operate-icon:hover {
    width: 20px;
    height: 20px;
...
.qb-page-tag {
  ...
  user-select: none;
  background: #fff;
  width: calc(100% - @qb-page-tag-operate-width);

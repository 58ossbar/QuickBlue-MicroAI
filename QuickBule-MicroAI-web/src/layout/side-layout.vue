<template>
  <a-layout class="admin-layout" style="min-height: 100%">
    ...
    <a-layout-sider
      ...
      class="side-menu"
      :class="`collapsed-show-${menuCollapsedShow}`"
      :width="sideMenuWidth"
    ...
    <a-layout :id="LAYOUT_ELEMENT_IDS.main" :style="`height: ${windowHeight}px`" class="admin-layout-main">
      ...
      <a-layout-header
        class="layout-header"
        :class="{ 'modern-layout-header': isModernTheme }"
...
<script setup>
  ...
  //折叠后菜单宽度
  const collapsedMenuWidth = computed(() => {
    const map = { icon: 64, text: 96, both: 120 };
    return map[menuCollapsedShow.value] ?? 64;
  });
  //主题颜色

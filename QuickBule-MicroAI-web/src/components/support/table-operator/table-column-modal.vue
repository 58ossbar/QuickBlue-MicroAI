...
<template>
  <a-modal :width="800" :open="visible" title="设置列" :destroyOnClose="true" :closable="false">
    <div v-if="!tableId">
      <a-alert type="error" show-icon class="qb-margin-bottom10">
        <template #message> 您尚未设置 TableOperator 组件的 tableId</template>
      </a-alert>
      <a-alert type="error" class="qb-margin-bottom10">
        <template #message>
    ...
    <div v-else>
      <a-alert type="info" show-icon class="qb-margin-bottom10">
        <template #icon><smile-outlined /></template>
        <template #message> 可以通过拖拽行直接修改顺序哦；（ <pushpin-outlined />为固定列，不可拖拽 ）</template>
      </a-alert>
      <a-table
        id="tableColumnModalTable"
        rowKey="columnKey"
      ...
      >
        <template #bodyCell="{ text, record, index, column }">
          ...
          <template v-if="column.dataIndex === 'operate'">
            <div class="qb-table-operate" v-if="!record.fixed">
...
<script setup>
  ...
  import { mergeColumn } from './table-column-merge';
  ...
  //初始化拖拽
  function initDrag() {
    let tbody = document.querySelector('#tableColumnModalTable tbody');
    Sortable.create(tbody, {
      animation: 300,
      dragClass: 'qb-ghost-class', //设置拖拽样式类名
      ghostClass: 'qb-ghost-class', //设置拖拽停靠样式类名
      chosenClass: 'qb-ghost-class', //设置选中样式类名
      handle: '.handle',

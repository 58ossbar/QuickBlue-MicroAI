      ...
      <slot>
        <template v-if="column.filterOptions">
          ...
          <template v-else-if="column.filterOptions.type === 'submit'">
            <div class="qb-table-operate"><a-button :type="column.filterOptions.btnType" @click="submit">查询</a-button></div>
          </template>
          <template v-else>
            <component
...
<script setup>
  ...
  const components = {
    'enum-select': defineAsyncComponent(() => import('/src/components/framework/enum-select/index.vue')),

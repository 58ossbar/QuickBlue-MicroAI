...
<h1 align="center">⚡ QuickBlue · 原生 AI 微服务应用底座</h1>

<p align="center">
  <strong>企业 AI 落地，先有底座 —— JDK 21 · Spring Cloud 2025 · Vite 8</strong>
  <br/>
  组织权限 · 数据安全 · 操作审计 · 服务监控 · AI 能力，一次搭好，直接在上面长业务
</p>
...
**QuickBlue** 是一款面向生产环境的**原生 AI 微服务快速开发平台**。基于 **JDK 21 (LTS) + Spring Cloud 2025** 与 **Vite 8 + Vue 3**，把"每个企业都要重写一遍"的那部分先做好——**组织权限、数据安全与审计、服务治理与监控**，再叠加开箱即用的 **AI 能力（多模型接入 / RAG 知识库 / 应用编排 / 会话）**。企业不必重复造地基，直接在上面长自己的 AI 业务。

> 🧩 **双数据库版本**：本仓库开源 **MySQL 版**（各微服务数据库隔离、独立账号）；官方同步提供 **PostgreSQL 版**（含 pgvector 向量检索），数据库层由 Nacos 配置中心集中管理，切换仅调配置，业务代码零侵入。

### 2.1 为什么企业需要一个"底座"

几乎所有企业都想融合 AI，但真正落地时，卡住的往往不是模型，而是**承载 AI 的应用底座**：

| 企业想做 AI | 实际会卡在哪 |
| --- | --- |
| 让 AI 回答业务问题 | 业务数据散在各个系统里，且**必须先过权限**——AI 不能看到不该看的数据 |
| 接一个大模型 | 模型、价格、合规政策都在变，**和业务代码绑死后就换不动** |
| 做个 AI 功能 Demo | Demo 一周能做，但**能被几百人稳定使用**（有账号、有审计、有监控、有运维）是另一回事 |
| 把客户数据喂给 AI | **数据出境、API Key 泄露、等保合规**没人敢签字 |
| 上线后持续迭代 | 没有组织架构、菜单权限、操作日志、备份恢复，系统越做越乱，**没人敢改** |

> 💡 **模型是发动机，底座是底盘。发动机可以换，底盘必须自己有。**

### 2.2 QuickBlue 的答案

不替代你的业务，只把地基先铺好：

- **权限与组织**：用户 / 角色 / 菜单 / 按钮 / 部门数据权限，AI 能力天然跑在这套边界之内
- **AI 能力层**：多模型中立接入、RAG 知识库、应用编排、多轮会话，模型可替换、可本地私有化
- **生产就绪**：服务监控、操作审计、数据库备份、定时任务、缓存、文件存储、Excel 导入导出
- **工程底座**：微服务拆分、统一网关、配置中心，一份 `.env` 全局生效

---

## 三、为什么企业选择 QuickBlue

按企业选型的真实决策顺序排列：**能不能跑起来 → 敢不敢用 → 多久出业务 → 长期谁维护**。

### ⚡ 1. 先看见再决策：10 分钟跑起来

企业选型最怕"PPT 很好、跑不起来"。QuickBlue 与绝大多数开源框架**最不一样的地方**就在这里——别人给你一堆 yml 和 SQL 让你自己拼装，QuickBlue 直接给你一个**浏览器安装向导**：**填地址 → 点"测试连接" → 下一步 → 开始安装**，全程鼠标操作，**不用敲命令、不用手写一行配置、不用手工粘贴 Nacos 配置**。
...
### 🔐 2. 安全与数据主权：企业敢不敢用的第一道门槛

Sa-Token 认证 + RBAC（用户 / 角色 / 菜单 / 按钮）+ 部门数据权限隔离 + 操作审计（`@AuditLog` 含 IP 归属地）+ API 加解密 + 数据脱敏 + 三级等保 + 密码安全策略（失败锁定）+ 防重复提交 + 全局异常统一处理。**整套可私有化部署，数据不出企业**，这是合规能签字的前提。

### 🤖 3. AI 长在业务与权限之上，不是又一个聊天框

- **RAG 知识库**：知识 / 记忆双类型，向量化检索，让 AI 回答"懂你的业务"，且**不越权**
- **应用编排**：开场白、预设问题、提示词、知识库关联、记忆开关、变量配置，像搭积木一样组装 AI 应用
- **会话管理**：多轮对话、Markdown 渲染、代码高亮，前端页面齐全

### 🔄 4. 模型中立：不被单一厂商绑定

OpenAI / 通义千问（DashScope）/ 智谱 / Ollama **统一接入，API Key 加密存储，一键激活切换**。供应商与业务代码解耦：今天用云上大模型，明天换国产模型或本地 Ollama 私有模型，**改配置不改代码**，成本与合规策略由企业自己掌握。

### 🏗 5. 真正的微服务，撑得住业务增长

网关 + 系统 + 业务 + 支撑 + AI **五大服务按领域边界拆分**，每个服务**独立数据库、独立账号、最小权限授权**。Spring Cloud Gateway 统一路由、白名单与 OpenAPI 3 文档聚合，后续加服务不必重构。

### ⚙️ 6. 一份配置，全局生效

MySQL / Redis / Nacos 等基础设施地址全部环境变量外部化，修改 **1 个 `.env` 文件**即可切换任意环境，告别"改 5 个 yml 才能换库"的痛。

### 📦 7. 那些"不酷但必须有"的能力，已经内置

EasyExcel 大数据量导入导出、S3 兼容对象存储、定时任务调度、二维码生成、单据序列号、数据脱敏、数据库备份、多级缓存（Caffeine + Redisson）全部内置，模板引擎（Velocity / FreeMarker）依赖已就位——省掉的是几个月的重复劳动。

### 🚀 技术底座：站在下一个十年

**JDK 21 (LTS)** 虚拟线程 + **Spring Boot 3.5.10** + **Spring Cloud 2025.0.1** + **Vite 8**。对比大量仍停留在 JDK 8 / Spring Boot 2.x / Webpack 的老牌框架，QuickBlue 从一开始就站在下一个十年的起跑线上——技术栈生命周期，管的正是未来 5~10 年的维护成本。

> 🤝 需要定制化方案、私有化部署或商业支持？见[商务合作](#contact)。
